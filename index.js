...
function getRandomFlavors(originalFlavors, currentFlavors, seasonalFlavors, regionalFlavors){
  ...
  for(let i = 0; i < 31; i++){
    ...
    if (randomArray === 0 ){
      randomArray = originalFlavors;
    } else if (randomArray === 1){
      randomArray = currentFlavors;
    } else if (randomArray === 2){
      randomArray = seasonalFlavors;
    } else {
      randomArray = regionalFlavors;
    }
    // Pick random flavor (index)
    let randomIndex = Math.floor(Math.random() * randomArray.length);
    // Add it to the array
    randomFlavors.push(randomArray[randomIndex]);
  }
